export default class Timer {
  constructor(root) {
    ...
    this.shortBreakSettingInput = document.getElementById('shortBreakSettingInput'); // Select the short break setting input element
    ...
    this.shortBreakSettingInput.addEventListener('keydown', (e) => {
      // Set the timer time when Enter key is pressed in the input field
      if (e.key === 'Enter') {
        this.setTime();
      }
  ...
  openModal() {
    ...
    this.shortBreakSettingInput.value = '';
  ...
  start() {
    ...
    this.interval = setInterval(() => {
      ...
      if (this.remainingSeconds === 0) {
        // Timer has reached zero, stop the timer, play the end sound, and start short break
        this.stop();
        this.playEndSound();
  ...
  // Start short break
  startShortBreak() {
    const shortBreakMinutes = parseFloat(this.shortBreakSettingInput.value);
    if (!isNaN(shortBreakMinutes) && shortBreakMinutes > 0) {
      this.stop();
      this.remainingSeconds = Math.floor(shortBreakMinutes * 60);
      this.updateInterfaceTime();
      this.playEndSound();
    }
  }
        ...
        <div class="modal__overlay" tabindex="-1">
          <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-rules-title">
            ...
            <article class="modal__content" id="modal-rules-content">
              <label for="timeSettingInput">Time (minutes)</label>
              <input type="number" step="0.1" id="timeSettingInput" class="time-setting" placeholder="Pomodoro">
              <input type="number" step="0.1" id="shortBreakSettingInput" class="time-setting" placeholder="Short Break">
              <button class="modal__btn" id="setTimeButton">Set Time</button>
            </article>
          </div>
        </div>
      </section>
